...
public class XCrypt {
	...
	static {
		String strOS = System.getProperty("os.name").replace(" ", "_").toLowerCase();
		String strProc = System.getProperty("os.arch").replace(" ", "_");

		if(strOS.contains("windows")){
			System.loadLibrary(XCrypt.class.getName() + "." + strProc);
		}else{
			if(strOS.contains("mac_os")) strOS = "macos";
			System.loadLibrary(XCrypt.class.getName() + "." + strOS + "." + strProc);
		}
		xCrypt = new XCrypt();
	}
